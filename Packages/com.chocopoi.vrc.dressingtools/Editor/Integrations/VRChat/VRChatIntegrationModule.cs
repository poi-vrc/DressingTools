...
namespace Chocopoi.DressingTools.Integration.VRChat.Modules
{
    ...
    internal class VRChatIntegrationModuleProvider : ModuleProviderBase
    {
        ...
        public static bool ApplyAnimationsAndMenu(ApplyCabinetContext ctx)
        {
            ...
            if (!ctx.cabinet.avatarGameObject.TryGetComponent<VRCAvatarDescriptor>(out var avatarDescriptor))
            {
                // not a vrc avatar
                return true;
            }
            ...
            for (var i = 0; i < wearables.Length; i++)
            {
                ...
                var animationGenerator = new AnimationGenerator(ctx.report, ctx.cabinet.avatarGameObject, agm, wearables[i].wearableGameObject, wearableDynamics);

                // TODO: merge disable clips and check for conflicts
                var wearAnimations = animationGenerator.GenerateWearAnimations(ctx.cabinet.animationGenerationWriteDefaults);
            ...
            AnimationUtils.GenerateAnyStateLayer(fxController, "cpDT_Cabinet", "cpDT_Cabinet", pairs, ctx.cabinet.animationGenerationWriteDefaults, null, refTransition);
