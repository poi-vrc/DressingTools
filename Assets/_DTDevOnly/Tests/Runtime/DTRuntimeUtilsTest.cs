...
namespace Chocopoi.DressingTools.Tests
{
    public class DTRuntimeUtilsTest : DTTestBase
    {
        ...
        public void FindType_ReturnsCorrectType()
        {
            ...
            var ret = DTRuntimeUtils.FindType(myDummyType.FullName);
            Assert.NotNull(ret);
        ...
        public void FindType_CacheCoverage()
        {
            ...
            var ret1 = DTRuntimeUtils.FindType(myDummyType.FullName);
            Assert.NotNull(ret1);
            Assert.AreEqual(myDummyType, ret1);

            var ret2 = DTRuntimeUtils.FindType(myDummyType.FullName);
            Assert.NotNull(ret2);
        ...
        {
            var ret = DTRuntimeUtils.FindType("Abababababa.No.Such.Class");
            Assert.Null(ret);
        }
        ...
        public void HandleBoneMappingOverrides_BoneOverrides1()
        {
            ...
            DTRuntimeUtils.HandleBoneMappingOverrides(newMappings, overrideMappings);
        ...
        public void GetRelativePath_ReturnsValidPath_WithoutUntilTransform()
        {
            ...
            string path = DTRuntimeUtils.GetRelativePath(child3.transform);
        ...
        public void GetRelativePath_ReturnsValidPath_WithUntilTransform()
        {
            ...
            string path = DTRuntimeUtils.GetRelativePath(child3.transform, child1.transform);
        ...
        public void GetRelativePath_ReturnsValidPath_WithPrefixSuffix()
        {
            ...
            string path = DTRuntimeUtils.GetRelativePath(child3.transform, child1.transform, "SomePrefix/", "/SomeSuffix");
        ...
        public void IsGrandParentTest()
        {
            ...
            //  |- Child2
            //
            var root = InstantiateRuntimeTestPrefab("IsGrandParentTestPrefab.prefab");
            ...
            // depth 0
            Assert.True(DTRuntimeUtils.IsGrandParent(root.transform, child1));
            Assert.True(DTRuntimeUtils.IsGrandParent(child1, grandChild1));
            Assert.True(DTRuntimeUtils.IsGrandParent(grandChild1, grandGrandChild1));
            Assert.True(DTRuntimeUtils.IsGrandParent(grandChild1, grandGrandChild2));
            Assert.True(DTRuntimeUtils.IsGrandParent(child1, grandChild2));
            Assert.True(DTRuntimeUtils.IsGrandParent(root.transform, child2));
            Assert.False(DTRuntimeUtils.IsGrandParent(child2, grandChild1));
            Assert.False(DTRuntimeUtils.IsGrandParent(child2, grandChild2));
            Assert.False(DTRuntimeUtils.IsGrandParent(grandChild2, grandGrandChild1));
            Assert.False(DTRuntimeUtils.IsGrandParent(grandChild2, grandGrandChild2));

            // depth 1
            Assert.True(DTRuntimeUtils.IsGrandParent(root.transform, grandChild1));
            Assert.True(DTRuntimeUtils.IsGrandParent(root.transform, grandChild2));
            Assert.True(DTRuntimeUtils.IsGrandParent(child1, grandGrandChild1));
            Assert.True(DTRuntimeUtils.IsGrandParent(child1, grandGrandChild2));
            Assert.False(DTRuntimeUtils.IsGrandParent(child2, grandGrandChild1));
            Assert.False(DTRuntimeUtils.IsGrandParent(child2, grandGrandChild2));

            // depth 2
            Assert.True(DTRuntimeUtils.IsGrandParent(root.transform, grandGrandChild1));
            Assert.True(DTRuntimeUtils.IsGrandParent(root.transform, grandGrandChild2));

            // Random object
            Assert.False(DTRuntimeUtils.IsGrandParent(randomObject.transform, root.transform));
            Assert.False(DTRuntimeUtils.IsGrandParent(randomObject.transform, child1));
            Assert.False(DTRuntimeUtils.IsGrandParent(randomObject.transform, grandChild1));
            Assert.False(DTRuntimeUtils.IsGrandParent(randomObject.transform, grandGrandChild1));
            Assert.False(DTRuntimeUtils.IsGrandParent(randomObject.transform, grandGrandChild2));
            Assert.False(DTRuntimeUtils.IsGrandParent(randomObject.transform, grandChild2));
            Assert.False(DTRuntimeUtils.IsGrandParent(randomObject.transform, child2));
        }

        private static void AssertScannedDynamics(GameObject root)
        {
            var excludeWearableDynamics = DTRuntimeUtils.ScanDynamics(root, true);
            Assert.AreEqual(2, excludeWearableDynamics.Count, "Should have 2 dynamics with wearable dynamics excluded");

            var includeWearableDynamics = DTRuntimeUtils.ScanDynamics(root, false);
        ...
        public void ScanDynamics_DynamicsBone()
        {
            ...
            var root = InstantiateRuntimeTestPrefab("DTTest_DynamicBoneAvatar.prefab");
            InstantiateRuntimeTestPrefab("DTTest_DynamicBoneWearable.prefab", root.transform);
...
#else
            var root = InstantiateRuntimeTestPrefab("DTTest_PhysBoneAvatar.prefab");
            InstantiateRuntimeTestPrefab("DTTest_PhysBoneWearable.prefab", root.transform);
            AssertScannedDynamics(root);
#endif
        }
        ...
        public void FindDynamicsWithRootTest()
        {
            ...
            var dynObj1 = DTRuntimeUtils.FindDynamicsWithRoot(list, obj1.transform);
            Assert.NotNull(dynObj1);
            Assert.AreEqual(obj1.transform, dynObj1.RootTransform);

            var dynObj2 = DTRuntimeUtils.FindDynamicsWithRoot(list, obj2.transform);
            Assert.NotNull(dynObj2);
            Assert.AreEqual(obj2.transform, dynObj2.RootTransform);

            var dynObj3 = DTRuntimeUtils.FindDynamicsWithRoot(list, obj3.transform);
            Assert.NotNull(dynObj3);
            Assert.AreEqual(obj3.transform, dynObj3.RootTransform);

            Assert.Null(DTRuntimeUtils.FindDynamicsWithRoot(list, obj4.transform));
        }
        ...
        public void IsDynamicsExistsTest()
        {
            ...
            Assert.True(DTRuntimeUtils.IsDynamicsExists(list, obj1.transform));
            Assert.False(DTRuntimeUtils.IsDynamicsExists(list, obj2.transform));
        }
        ...
        public void GuessMatchingAvatarBoneTest()
        {
            ...
            // remove prefix
            trans = DTRuntimeUtils.GuessMatchingAvatarBone(root.transform, "(SomePrefix) LeftUpperLeg");
            Assert.NotNull(trans);
            Assert.AreEqual(bone1.transform, trans);

            // remove suffix
            trans = DTRuntimeUtils.GuessMatchingAvatarBone(root.transform, "LeftUpperLeg (SomeSuffix)");
            Assert.NotNull(trans);
            Assert.AreEqual(bone1.transform, trans);

            // trim and remove prefix and suffix
            trans = DTRuntimeUtils.GuessMatchingAvatarBone(root.transform, "  (SomePrefix)   LeftUpperLeg(SomeSuffix)    ");
            Assert.NotNull(trans);
            Assert.AreEqual(bone1.transform, trans);

            // guess bone using bone mapping names list
            trans = DTRuntimeUtils.GuessMatchingAvatarBone(root.transform, "Leg_L");
            Assert.NotNull(trans);
            Assert.AreEqual(bone1.transform, trans);

            trans = DTRuntimeUtils.GuessMatchingAvatarBone(root.transform, "Right leg");
            Assert.NotNull(trans);
            Assert.AreEqual(bone2.transform, trans);

            // no such bone
            trans = DTRuntimeUtils.GuessMatchingAvatarBone(root.transform, "Baka bone");
            Assert.Null(trans);
        }
        ...
        public void GuessArmature_PrefixedSuffixed()
        {
            ...
            var trans = DTRuntimeUtils.GuessArmature(root, "Armature");
            Assert.NotNull(trans);
        ...
        public void GuessArmature_Rename()
        {
            var root = CreateGameObject("GuessArmatureRoot1");
            var armature = CreateGameObject("aRmaTURe", root.transform);
            var trans = DTRuntimeUtils.GuessArmature(root, "Armature", true);
            Assert.NotNull(trans);
            Assert.AreEqual(armature.transform, trans);
        ...
        public void CopyComponentTest()
        {
            ...
            var copiedComp = DTRuntimeUtils.CopyComponent(comp, obj2);
        ...
        public void IsOriginatedFromAnyWearableTest()
        {
            ...
            var wearableRoot = InstantiateRuntimeTestPrefab("DTTest_PhysBoneWearable.prefab", avatarRoot.transform);
            ...
            Assert.True(DTRuntimeUtils.IsOriginatedFromAnyWearable(avatarRoot.transform, wearableRootArmature));
            Assert.False(DTRuntimeUtils.IsOriginatedFromAnyWearable(avatarRoot.transform, avatarRootArmature));
        }
    }
}
