...
namespace Chocopoi.DressingTools.Dresser.Default
{
    internal class DefaultDresser : IDresser
    {
        ...
        private static DynamicsOptions DetectDynamicsOption(Report report, Transform avatarRoot, Transform sourceTrans, Transform targetTrans)
        {
            ...
            {
                report.LogInfo(LogLabel, $"Frankenstein bone detected, using IgnoreTransform instead: {AnimationUtils.GetRelativePath(sourceTrans, avatarRoot)} -> {AnimationUtils.GetRelativePath(targetTrans, avatarRoot)}");
        ...
        private static void AddTagDownwards<T>(Transform avatarRoot, Transform sourceTrans, Transform targetTrans, List<ITag> tags) where T : ITag, new()
        {
            // add tag
            var tag = new T
            {
                SourceTransform = sourceTrans,
                // TODO: remove this later on, just for temporary migration
                TargetPath = AnimationUtils.GetRelativePath(targetTrans, avatarRoot)
            };
            tags.Add(tag);
            ...
            for (var i = 0; i < sourceTrans.childCount; i++)
            {
                ...
                if (avatarTrans != null)
                {
                    AddTagDownwards<T>(avatarRoot, child, avatarTrans, tags);
                }
            }
        }

        private static void HandleBoneDynamics(DynamicsOptions option, Transform avatarRoot, Transform sourceTrans, Transform targetTrans, IDynamics sourceDynamics, List<ObjectMapping> objectMappings, List<ITag> tags)
        {
            ...
            {
                AddTagDownwards<IgnoreDynamicsTag>(avatarRoot, sourceTrans, targetTrans, tags);
            }
        ...
        private static void ProcessBone(Report report, DynamicsOptions dynamicsOption, Transform avatarRoot, Transform sourceArmatureParent, Transform targetArmatureParent, List<IDynamics> allDynamics, List<ObjectMapping> objectMappings, List<ITag> tags, int level)
        {
            for (var i = 0; i < sourceArmatureParent.childCount; i++)
            {
                ...
                HandleBoneDynamics(option, avatarRoot, child, targetTrans, sourceDynamics, objectMappings, tags);
            }
        }
