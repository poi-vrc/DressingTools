    ...
    internal class CopyDynamicsPass : ComponentPass
    {
        ...
        private static Dictionary<DTCopyDynamics, Component> MakeCopies(Context ctx, DTCopyDynamics[] copyDynComps)
        {
            ...
            foreach (var copyDynComp in copyDynComps)
            {
                ...
                {
                    ctx.Report.LogWarn("CopyDynamicsPass", $"Dynamics not found for copying at path, ignoring: {copyDynComp.SearchPath} in mode {copyDynComp.SearchMode}");
                    continue;
                }
        ...
        {
            var targetRoot = string.IsNullOrEmpty(comp.SearchPath) ? root : root.Find(comp.SearchPath);

            var allDynamics = DynamicsUtils.ScanDynamics(root.gameObject);

            if (comp.SearchMode == DTCopyDynamics.DynamicsSearchMode.ControlRoot)
            {
                return allDynamics.Where(d => d.RootTransforms.Contains(targetRoot)).FirstOrDefault();
            }
            else if (comp.SearchMode == DTCopyDynamics.DynamicsSearchMode.ComponentRoot)
            {
